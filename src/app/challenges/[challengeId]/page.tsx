...
interface Challenge { challengeId: string; title: string; problem: string; constraints: string; inspirationDomains: string[]; sources: Source[]; status: string }
...
const SC: Record<string, string> = { open: '#22c55e', completed: '#52525b', upcoming: '#8b5cf6' }
const SL: Record<string, string> = { open: 'Open', completed: 'Completed', upcoming: 'Upcoming' }
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '60px 24px' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
          <span style={{ fontSize: '11px', fontWeight: 600, padding: '3px 10px', borderRadius: '20px', border: `1px solid ${SC[challenge.status] ?? '#52525b'}44`, backgroundColor: `${SC[challenge.status] ?? '#52525b'}10`, color: SC[challenge.status] ?? '#52525b', letterSpacing: '0.03em' }}>
            {SL[challenge.status] || challenge.status}
          </span>
          {proposals.length > 0 && (
            <span style={{ fontSize: '12px', color: '#52525b' }}>{proposals.length} proposal{proposals.length !== 1 ? 's' : ''}</span>
          )}
        </div>
      ...
      {challenge.sources?.length > 0 && (
        <div style={{ ...glass, padding: '22px 28px', marginBottom: '24px' }}>
      ...
      {/* Submit CTA — always shown unless completed */}
      {challenge.status !== 'completed' && (
        <div style={{ background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.15)', borderRadius: '14px', padding: '20px 28px', marginBottom: '32px' }}>
          <p style={{ fontSize: '13px', fontWeight: 600, color: '#22c55e', marginBottom: '6px' }}>
            Submit a proposal · vote on others
          </p>
      ...
      <section>
        ...
        {proposals.length === 0 ? (
          <p style={{ color: '#52525b', fontSize: '14px' }}>No proposals yet. Be the first.</p>
        ) : (
